...
// TMDB Data Classes
data class CityPlexTmdbSearchResponse(
    @param:JsonProperty("results") val results: List<CityPlexTmdbSearchResult>? = null
)

data class CityPlexTmdbSearchResult(
...
data class CityPlexTmdbDetails(
    @param:JsonProperty("overview") val overview: String? = null,
    @param:JsonProperty("poster_path") val posterPath: String? = null,
    @param:JsonProperty("vote_average") val rating: Double? = null,
    @param:JsonProperty("release_date") val releaseDate: String? = null
...
data class CityPlexTmdbEpisodeDetails(
...
data class CityPlexTmdbSeasonDetails(
    @param:JsonProperty("episodes") val episodes: List<CityPlexTmdbEpisodeDetails>? = null
)

data class CityPlexTmdbExternalIds(
    @param:JsonProperty("imdb_id") val imdbId: String? = null
)

object CityPlexTmdbHelper {
    private const val TMDB_API = "https://api.themoviedb.org/3"
    private const val TMDB_IMAGE_BASE_URL = "https://image.tmdb.org/t/p"
    private const val TAG = "CityPlexTmdbHelper"
    ...
    private fun getApiKey(): String {
        return try {
            CityPlexSettingsManager.getApiKey() ?: return ""
        } catch (e: Exception) {
    ...
    suspend fun searchTmdb(title: String, isMovie: Boolean = true): CityPlexTmdbSearchResult? {
        if (!CityPlexSettingsManager.isTmdbEnabled()) {
        ...
        return try {
            ...
            val results = parseJson<CityPlexTmdbSearchResponse>(response)
    ...
    suspend fun getTmdbDetails(id: Int, isMovie: Boolean = true): CityPlexTmdbDetails? {
        if (!CityPlexSettingsManager.isTmdbEnabled()) {
    ...
    suspend fun getImdbIdFromTmdb(
        ...
        return try {
            val response = makeApiCall(url) ?: return null
            parseJson<CityPlexTmdbExternalIds>(response).imdbId
        } catch (e: Exception) {
    ...
    suspend fun getAllSeasonDetails(
        ...
        val results = mutableMapOf<Int, CityPlexTmdbSeasonDetails?>()
