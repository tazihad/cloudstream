...
// TMDB Data Classes
data class CtgHallTmdbSearchResponse(
    @param:JsonProperty("results") val results: List<CtgHallTmdbSearchResult>? = null
)

data class CtgHallTmdbSearchResult(
...
data class CtgHallTmdbEpisodeDetails(
...
data class CtgHallTmdbSeasonDetails(
    @param:JsonProperty("episodes") val episodes: List<CtgHallTmdbEpisodeDetails>? = null
)

data class CtgHallTmdbExternalIds(
    @param:JsonProperty("imdb_id") val imdbId: String? = null
)

object CtgHallTmdbHelper {
    private const val TMDB_API = "https://api.themoviedb.org/3"
    private const val TMDB_IMAGE_BASE_URL = "https://image.tmdb.org/t/p"
    private const val TAG = "CtgHallTmdbHelper"
    ...
    private fun getApiKey(): String {
        return try {
            CtgHallSettingsManager.getApiKey() ?: return ""
        } catch (e: Exception) {
    ...
    suspend fun searchTmdb(title: String, isMovie: Boolean = true): CtgHallTmdbSearchResult? {
        if (!CtgHallSettingsManager.isTmdbEnabled()) {
        ...
        return try {
            ...
            val results = parseJson<CtgHallTmdbSearchResponse>(response)
    ...
    suspend fun getTmdbDetails(id: Int, isMovie: Boolean = true): CtgHallTmdbDetails? {
        if (!CtgHallSettingsManager.isTmdbEnabled()) {
    ...
    suspend fun getImdbIdFromTmdb(
        ...
        return try {
            val response = makeApiCall(url) ?: return null
            parseJson<CtgHallTmdbExternalIds>(response).imdbId
        } catch (e: Exception) {
    ...
    suspend fun getAllSeasonDetails(
        ...
        val results = mutableMapOf<Int, CtgHallTmdbSeasonDetails?>()
    ...
    fun getEpisodeFromSeasonData(
        seasonData: CtgHallTmdbSeasonDetails?,
        episodeNumber: Int
    ): CtgHallTmdbEpisodeDetails? {
        return seasonData?.episodes?.find { it.episodeNumber == episodeNumber }
    }

    fun getEpisodeDetailsFromAnySeason(
        bulkSeasonData: Map<Int, CtgHallTmdbSeasonDetails?>,
        seasonNumber: Int,
        episodeNumber: Int
    ): CtgHallTmdbEpisodeDetails? {
